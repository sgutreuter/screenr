...
##
#################################################################################
#' Test-Screening Tool Based on Generalized Estimating Equations Binomial Models
#'
#' Estimation of binomial model parameters in the presence of cluster-level
#' random effects, and cross-validated performance of test screening based on
#' Generalized Estimating Equations.  The results provide information from which
#' to choose a probability threshold above which individual out-of-sample
...
#' @param corstr a character string specifying the correlation structure among
#' the sampling clusters.  The permissible values are: \code{"independence"},
#' \code{"fixed"}, \code{"stat_M_dep"}, \code{"non_stat_M_dep"},
#' \code{"exchangeable"}, \code{"AR-M"} and \code{"unstructured"}.  See
#' \code{\link{gee}} for details.
...
#' data(unicorns)
#' help(unicorns)
#' unitool <- geeScreening(testresult ~ Q1 + Q2 + Q3 + Q4 + Q5,
...
geeScreening <- function(formula,
    ...
    lrfit <- gee::gee(formula, id = id, corstr = corstr, data = dat, family = binomial(link = link))
    ...
    for(i in 1:Nfolds){
        res <- gee::gee(formula, id = id, corstr = corstr, data = dat[-holdouts[[i]], ],
