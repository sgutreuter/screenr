...
predictor covariates automatically to prevent over-fitting, robust to
...
predictor variables `Q1`, ..., `Q7`. The argument `Nfolds = 10`
...
effects of any strong correlations among the predictors which provides
...
If the chosen screening threshold was 0.0066, then testing Bernie P.
...
should be approximately 0.0006 (0.06\%). In contrast, the prevalence
of UIV in the training data is `r (pbar <- mean(unicorns$testresult,
na.rm = TRUE))` and therefore we would, on average, need to test
approximately `r round(1/pbar)` unicorns in order to detect a single
infection.
