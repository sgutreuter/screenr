...
is evaluated using receiver-operating characteristics
...
A sample of \Sexpr{dim(unicorns)[1]} properly consented adult unicorns
...
<<echo=F>>=
prev_u <- round(mean(unicorns$testresult, na.rm = TRUE), digits = 3)
@
The prevalence of UIV in the sample of unicorns is
\Sexpr{prev_u}. Under universal testing
at health-care entry points, discovery of a single
infection would require, on average, testing
\Sexpr{round(1 / prev_u)} unicorns.

\section*{Screening Tool Development}

This demonstration of screening-tool development and thorough
validation consists of four steps:
\begin{enumerate}
...
model from the training data.  A method known as \emph{L}1
regularization has desirable properties for that
task\citep{Hastie+al2009,Park+Hastie2007}.  The function
\textsf{lasso\_screenr()} provides easy, integrated access to the
\emph{L}1 regularization algorithm of Park and
...
lasso\citep{Hastie+al2009}.  Unlike the conventional lasso, the
...
applications).  Two solutions along the regularization path which
yield the smallest
...
useful solutions in nearly all settings. However all solutions are
accessible.
...
the fits to all of the data and also using \emph{k}-fold cross validation.
...
changed along the regularization path for either of the special fits
by extracting the regularization path object.
<<>>=
pathobj <- get_what(from = uniobj1, what = "glmpathObj", model = "minAIC")
@
\begin{figure}[!h]
  \begin{center}
<<fig=T, pdf=T, png=T, echo=T, width=4, height=4>>=
plot(pathobj)
@
...
\end{center}
\end{figure}
Figure \ref{fig:f1} is produced using the plot method.  The horizontal
axis is the \emph{L}1-norm of the coefficient vector and the vertical
axis shows values of the individual coefficients (see
\citep{Park+Hastie2007}
for details).

The \textsf{screenr} package provides the following methods for
...
might choose to re-fit the selected model using ordinary logistic
modeling as implemented in \textsf{logreg\_screenr}.  We can refit the
...
which was selected using regularization are obtained using:
<<>>=
confint(uniobj2)
@
...
\end{center}
\end{figure}
to produce Figure \ref{fig:f2}.  Both the overly-optimistic in-sample ROC and the
cross-validated out-of-sample ROC from the AIC-best model
are shown. 95\% confidence intervals for sensitivity and specificity are plotted
at the local maxima (largest distances perpendicular to the 1:1
line). Those maxima are the complete set options for screening.  A
...
ROC curve for which sensitivity is at least 0.8.
...
\textbf{Therefore it is highly desirable to validate the
...
the screening sample. A total of \Sexpr{dim(val_data)[1]} new unicorns
...
from the initial study, as contained in \textsf{uniobj2}.

The data frame \textsf{new\_preds} contains the results of tests from the new
...
\textsf{roc}-class objects differs from usage in the \textsf{screenr} package:
\begin{figure}[!h]
  \begin{center}
<<fig=T, pdf=T, png=T, echo=T, width=4, height=4>>=
...
the `predict` method.  The predicted probability of infection is given
by
\[P = \frac{1}{1 + exp(-\mathbf{Xb})}\] where $\mathbf{X}$ is a
...
testing based on the internally computed value of \emph{P}. This is the
ideal approach, and should be used wherever possible.

\textbf{NOTE:} Screening questions for which estimated coefficients equal 0
...
whole numbers between 1 and \emph{m}, where \emph{m} is some number from, say, 3
...
weights.  The choice of \emph{m} involves a trade-off. Small values, say
...
specificity. Large values for \emph{m} may approach the performance of
...
<<>>=
et_3 <- easy_tool(uniobj2, max = 3, crossval = TRUE)
class(et_3)
@
...
\end{center}
\end{figure}
Note that the AUC (Fig. \ref{fig:f4}) is almost as large as the AUC from the actual model
fit. However there are fewer local maximas, and therefore fewer
choices for screening thresholds. The screening thresholds for
the scores based on the sums of the question weights can also be printed:
<<>>=
...
\end{center}
\end{figure}
An example of a simple clinical screening form based on question
weights is shown in Fig. \ref{fig:f5}.

\textbf{NOTE:} Screening questions for which the question weights equal 0
...
<<echo=F>>=
pbar <-  round(mean(unicorns$testresult, na.rm = TRUE),
digits = 4)
@
...
\Sexpr{round(1/pbar, digits = 1)} would need to be tested in order to
