...
%%  Customizations:
\usepackage{Sweave}
\SweaveOpts{width=4,height=4,prefix=TRUE,prefix.string=FIXME}
...
If the chosen screening threshold was \Sexpr{roc_maximas[4,1]}, then
testing Bernie P.  but screening out Alice D. would have point
estimates of sensitivity and specificity of \Sexpr{roc_maximas[4,2]}
and \Sexpr{roc_maximas[4,5]} if the data from those two unicorns came
...
\begin{figure}[!h]
  \begin{center}
<<fig=T, pdf=T, png=T, echo=F, width=4, height=4>>=
plot(new_roc, print.auc = TRUE, partial.auc =  c(0.8, 1.0), type = "S",
     partial.auc.focus = "sensitivity", partial.auc.correct = TRUE)
@
