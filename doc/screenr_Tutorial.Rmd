...
model from the training data.  A method known as *L*1 regularization has desirable
...
*L*1-[norm](https://towardsdatascience.com/norms-penalties-and-multitask-learning-2f1db5f97c1f)of
their coefficients, similar to the ordinary lasso [2].  Unlike the
conventional lasso, the algorithm also imposes a very small penalty on
...
unlikely in test-screening applications).  Two solutions along the
...
modeling as implemented in `logreg_screenr`.  We can refit the
...
Again, the logit-scale maximum-likelihood estimates from the AIC-best model
...
The partial area under the curve (partial AUC) is good compared
